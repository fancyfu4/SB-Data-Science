...
LEFT JOIN Facilities AS f
	ON f.facid = b.facid
WHERE f.name = 'Tennis Court 1' OR 'Tennis Court 2'
ORDER BY membername
...
SELECT 
	...
	subquery.cost
FROM
	(SELECT m.firstname, m.surname, f.name, b.starttime,
    	CASE WHEN m.firstname = 'GUEST' THEN f.guestcost*b.slots ELSE f.membercost*b.slots END AS cost
	FROM Members AS m
    LEFT JOIN Bookings AS b
    	ON m.memid = b.memid
...
SELECT f.name,
	ROUND(SUM(CASE WHEN b.memid=0 THEN f.guestcost * b.slots ELSE f.membercost * b.slots END),2) AS revenue
...
SELECT
    ...
    (r.surname||", "||r.firstname) AS 'recommendingmember'
FROM Members AS m
...
SELECT f.name, COUNT(memid) AS 'usage'
FROM Bookings as b
LEFT JOIN Facilities as f
	ON b.facid = f.facid
WHERE b.memid !=0
...
SELECT f.name, COUNT(memid) AS 'usage', strftime('%m', starttime) as 'month'
FROM Bookings as b
LEFT JOIN Facilities as f
	ON b.facid = f.facid
WHERE b.memid !=0
